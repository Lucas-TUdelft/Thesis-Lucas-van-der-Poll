...
deadband_values = [np.deg2rad(2.0),np.deg2rad((8.0 / (7000 ** 2)))]
# bank angle guidance
aerodynamic_guidance_object = Util.ApolloGuidance.from_file(
    'apollo_data_vref.npz', bodies, deadband_values, estimated_flight_time, K=1.0)
...
speed = 6.93E3
heading_angle = np.deg2rad(95.25)
...
initial_vehicle_position_bodyfixed_unit = initial_cartesian_state_body_fixed[0:3] / np.linalg.norm(
    initial_cartesian_state_body_fixed[0:3])
dot_product = np.dot(final_vehicle_position_bodyfixed_unit, initial_vehicle_position_bodyfixed_unit)
dot_product = np.clip(dot_product, -1.0, 1.0)
downrange_distance = Earth_radius * np.arccos(dot_product)
print(downrange_distance / 1000)
...
ax1.scatter(longitude, latitude, s=2, transform=ccrs.PlateCarree())
ax1.plot(np.rad2deg(station_longitude), np.rad2deg(station_latitude), color = 'g', marker = '*', transform=ccrs.PlateCarree())
