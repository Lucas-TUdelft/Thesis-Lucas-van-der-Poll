...
max_final_velocity_constraint = 600 # m/s

inputs = np.empty((num_simulations),dtype=object)
objectives = np.empty((num_simulations),dtype=object)
constraints = np.empty((num_simulations),dtype=object)

parameters = np.empty((num_parameters,num_simulations),dtype=float)

for i in range(num_parameters):
    range_of_parameter = np.random.uniform(variation_range_per_parameter[i][0],
                                           variation_range_per_parameter[i][1],
                                           num_simulations)
    parameters[i] = range_of_parameter
...
for i in range(num_simulations):
    ...
    for j in range(num_parameters):
        default_inputs[j] = default_inputs[j] + parameters[j][i]
    ...
    inputs[i] = default_inputs
    objectives[i] = [final_distance_to_target, mp, n_bank_reversals]
    constraints[i] = [max_gload, max_heatflux, total_heatload, final_velocity, succesfull_completion]

within_constraints = []
outside_constraints = []

for i in range(num_simulations):
    within_gload = constraints[i][0] < max_g_constraint
    within_heatflux = constraints[i][1] < max_heatflux_constraint
    within_total_heatload = constraints[i][2] < max_total_heatload_constraint
    within_final_velocity = constraints[i][3] < max_final_velocity_constraint
    succesfull_completion = constraints[i][4]

    if within_gload and within_heatflux and within_total_heatload and within_final_velocity and succesfull_completion:
        within_constraints.append([inputs[i], objectives[i], constraints[i]])
    else:
        outside_constraints.append([inputs[i], objectives[i], constraints[i]])

savedata = [within_constraints, outside_constraints]
data_path = os.path.join(output_folder, target_location + '.dat')

with open(data_path,'wb') as f:
    pickle.dump(savedata, f)

if plot_results:
    input_within_constraints = [within_constraints[i][0] for i in range(len(within_constraints))]
    input_outside_constraints = [outside_constraints[i][0] for i in range(len(outside_constraints))]

    objective1_within_constraints = [within_constraints[i][1][0] for i in range(len(within_constraints))]
    objective2_within_constraints = [within_constraints[i][1][1] for i in range(len(within_constraints))]
    objective3_within_constraints = [within_constraints[i][1][2] for i in range(len(within_constraints))]

    objective1_outside_constraints = [outside_constraints[i][1][0] for i in range(len(outside_constraints))]
    objective2_outside_constraints = [outside_constraints[i][1][1] for i in range(len(outside_constraints))]
    objective3_outside_constraints = [outside_constraints[i][1][2] for i in range(len(outside_constraints))]

    gload_plots_within_constraints = [within_constraints[i][2][0] for i in range(len(within_constraints))]
    heatflux_plots_within_constraints = [within_constraints[i][2][1] for i in range(len(within_constraints))]
    heatload_plots_within_constraints = [within_constraints[i][2][2] for i in range(len(within_constraints))]
    final_velocity_plots_within_constraints = [within_constraints[i][2][3] for i in range(len(within_constraints))]

    gload_plots_outside_constraints = [outside_constraints[i][2][0] for i in range(len(outside_constraints))]
    heatflux_plots_outside_constraints = [outside_constraints[i][2][1] for i in range(len(outside_constraints))]
    heatload_plots_outside_constraints = [outside_constraints[i][2][2] for i in range(len(outside_constraints))]
    final_velocity_plots_outside_constraints = [outside_constraints[i][2][3] for i in range(len(outside_constraints))]

    for i in range(num_parameters):
        parameter_input_within_constraints = [input_within_constraints[j][i] for j in
                                              range(len(input_within_constraints))]
        parameter_input_outside_constraints = [input_outside_constraints[j][i] for j in
                                              range(len(input_outside_constraints))]
        ...
        # objectives
        fig.suptitle(
            'Parameter ' + parameternames[i] + ' against objectives, for ' + target_location)
        axs[0].scatter(parameter_input_within_constraints, objective1_within_constraints, color='blue',
                       label='Within constraints')
        axs[0].scatter(parameter_input_outside_constraints, objective1_outside_constraints, color='red',
                       label='Outside constraints')
        axs[0].set_title('Final distance to target')
        axs[0].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[0].set_ylabel('Final distance to target [m]')
        axs[0].legend()
        axs[0].grid()
        axs[1].scatter(parameter_input_within_constraints, objective2_within_constraints, color='blue',
                       label='Within constraints')
        axs[1].scatter(parameter_input_outside_constraints, objective2_outside_constraints, color='red',
                       label='Outside constraints')
        axs[1].set_title('Propellant mass')
        axs[1].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[1].set_ylabel('Propellant mass [kg]')
        axs[1].legend()
        axs[1].grid()
        axs[2].scatter(parameter_input_within_constraints, objective3_within_constraints, color='blue',
                       label='Within constraints')
        axs[2].scatter(parameter_input_outside_constraints, objective3_outside_constraints, color='red',
                       label='Outside constraints')
        axs[2].set_title('Number of bank reversals')
        axs[2].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[2].set_ylabel('number of bank reversals [-]')
        axs[2].legend()
        axs[2].grid()
        plt.tight_layout()
        figname = 'Parameter ' + parameternames[i] + ' MC all objectives ' + target_location + '.png'
        ...
        fig.suptitle(
            'Parameter ' + parameternames[i] + ' against constraints, for ' + target_location)
        axs[0].scatter(parameter_input_within_constraints, gload_plots_within_constraints, color='blue',
                       label='Within constraints')
        axs[0].scatter(parameter_input_outside_constraints, gload_plots_outside_constraints, color='red',
                       label='Outside constraints')
        axs[0].set_title('Max G-load')
        axs[0].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[0].set_ylabel('Max G-load [-]')
        axs[0].grid()
        axs[1].scatter(parameter_input_within_constraints, heatflux_plots_within_constraints, color='blue',
                       label='Within constraints')
        axs[1].scatter(parameter_input_outside_constraints, heatflux_plots_outside_constraints, color='red',
                       label='Outside constraints')
        axs[1].set_title('Max heat flux')
        axs[1].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[1].set_ylabel('Max heat flux [W/m^2]')
        axs[1].grid()
        axs[2].scatter(parameter_input_within_constraints, heatload_plots_within_constraints, color='blue',
                       label='Within constraints')
        axs[2].scatter(parameter_input_outside_constraints, heatload_plots_outside_constraints, color='red',
                       label='Outside constraints')
        axs[2].set_title('Max heat load')
        axs[2].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[2].set_ylabel('Max heat load [J/m^2]')
        axs[2].grid()
        axs[3].scatter(parameter_input_within_constraints, final_velocity_plots_within_constraints, color='blue',
                       label='Within constraints')
        axs[3].scatter(parameter_input_outside_constraints, final_velocity_plots_outside_constraints, color='red',
                       label='Outside constraints')
        axs[3].set_title('Final velocity')
        axs[3].set_xlabel('Parameter ' + parameternames_axis[i])
        axs[3].set_ylabel('Final velocity [m/s]')
        axs[3].grid()
        plt.tight_layout()
        figname = 'Parameter ' + parameternames[i] + ' MC all constraints ' + target_location + '.png'
        fig.savefig(os.path.join(output_folder, figname))
        plt.show()